          ...
          <div className="modal-header">
            <h5 className="modal-title" style={{width: '100%'}}>
              ...
              <div className='btn-group loadtype-buttons'>
                <button className={`btn btn-sm link-${this.state.activeLoadType === LoadType.Roster ? 'success' : 'danger'}`} onClick={this.setLoadType(LoadType.Roster)}>Roster</button>
                <button className={`btn btn-sm link-${this.state.activeLoadType === LoadType.Timings ? 'success' : 'danger'}`} onClick={this.setLoadType(LoadType.Timings)}>Timings</button>
                <button className={`btn btn-sm link-${this.state.activeLoadType === LoadType.Fight ? 'success' : 'danger'}`} onClick={this.setLoadType(LoadType.Fight)}>Fight</button>
              </div>
            </h5>
